...
class Runs:
    ...
    def get_runs(self, **kwargs):
        ...
        Na = np.nan
        df = df[['id', 'uuid', 'start','responded','created_on', 'modified_on','exited_on', 'exit_type', 'flow_uuid','contact_uuid','results']]
        df['timeout_on'], df['flowlabel_id'], df['current_node_uuid'], df['message_ids'], df['path'], df['session_id'], df['submitted_by_id'], df['is_active'], df['fields'], df['expires_on'], df['contact_id'], df['flow_id'], df['parent_id'], df['connection_id']= Na, Na,Na, Na,Na, Na,Na, Na,Na, Na,Na, Na,Na, Na
